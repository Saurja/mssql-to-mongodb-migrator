...
async function replicateTable(
  ...
  console.log(
    "Replicating " + tableName + " with primary key " + primaryKeyField
  );
  ...
  console.log(
    "Got " + tableResult.recordset.length + " records from table " + tableName
  );
  ...
  const bulkRecordInsert = E.from(tableResult.recordset)
    .select((row) => {
      if (config.remapKeys) {
        ...
        primaryKeyRemap.push({
          // Create a remap table so we can fixup foreign keys.
          new: row._id,
  ...
  if (config.remapKeys) {
    const primaryKeyRemapCollection = targetDb.collection(
      tableName + "-pkremap"
    );
...
async function remapForeignKeys(tableName, foreignKeysMap, targetDb, sqlPool) {
  ...
  for (const record of records) {
    ...
    thisCollection.updateOne({ _id: record._id }, { $set: foreignKeyUpdates });
  }
}
...
async function main() {
  ...
  const primaryKeyMap = E.from(primaryKeysResult.recordset).toObject(
    (row) => row.TABLE_NAME,
    (row) => row.COLUMN_NAME
  );
  ...
  const tablesResult = await sqlPool
    .request()
    .query(
  ...
  if (config.remapKeys) {
    ...
    const foreignKeyMap = E.from(foreignKeysResult.recordset)
      ...
      .select((group) => {
        return {
          table: group.key(),
          foreignKeys: E.from(group.getSource()).toObject(
            (row) => row.FK_Column,
            (row) => ({
              table: row.PK_Table,
